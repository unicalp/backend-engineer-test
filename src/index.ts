...
fastify.post('/rollback', async (request, reply) => {
  // Get the ?height=... query parameter
  ...
  try {
    ...
    // Find blocks to roll back (from newest to oldest)
    ...
    if (blocksToRollbackResult.rows.length === 0) {
      // Return a 400 (Bad Request) if there's nothing to roll back
      return reply.status(400).send({ error: `Already at or below height ${targetHeight}. No blocks to rollback.` });
    }
    ...
    for (const row of blocksToRollbackResult.rows) {
      ...
      for (const tx of block.transactions) {
        ...
        // 2a. Undo Outputs
        // Delete all UTXOs created by this transaction
        await client.query(
          'DELETE FROM utxos WHERE tx_id = $1',
          [tx.id]
        );

        // Subtract balances from the addresses that received them
        ...
        // 2b. Undo Inputs
        // Mark all UTXOs spent by this transaction as "unspent" again
        for (const input of tx.inputs) {
          // We need to find the original address and value of the UTXO being spent
          const utxoResult = await client.query(
